...
char shiftCharacter(const char& c, const int& key, const bool& encrypt)
{
    ...
    return static_cast<char>(base + (c - base + direction * key + 26) % 26);
}

std::string shiftCipher(const std::string& text, const int& key, const bool& encrypt)
{
...
std::vector<std::string> shiftCipherBruteForce(const std::string& cipherText)
{
    return std::views::iota(1, 26) | std::ranges::views::transform([&cipherText](int key) {
...
{
    const std::vector<std::string> results = shiftCipherBruteForce(cipherText);
    std::ranges::for_each(results, [i = 1](const std::string& result) mutable {
